...
public record QtfCompilationUnit(Class<?> compiledType, Map<String, Integer> inputs, List<String> outputs, Map<String, FunctionAddress> functions) {
    ...
    public QtfScript createScript(final @Nullable QtfListener listener) throws InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        if (listener != null) {
            listener.listen(Variable.resolve(outputs, runnable), outputs::stream);
        }
        return runnable;
    }
