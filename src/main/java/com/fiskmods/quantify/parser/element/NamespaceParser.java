...
class NamespaceParser implements SyntaxParser<NamespaceStatement> {
    ...
    @Override
    public NamespaceStatement accept(final QtfParser parser) throws QtfParseException {
        parser.startTree();
        parser.next(TokenClass.NAMESPACE);
        final Expression expression = ExpressionParser.INSTANCE.accept(parser);
